...
class GetItemCodeMasterApi {
  static Future<GetItemCodeModel> getData(int docEntry) async {
    int resCode = 500;
    ...
    try {
      ...
      log("GetAuditItem sts:::" "${response.statusCode.toString()}");
      // log("GetItem Res:::" "${response.body.toString()}");

      resCode = response.statusCode;
      if (response.statusCode == 200) {
        return GetItemCodeModel.fromJson(
            json.decode(response.body) as Map<String, Object?>,
            response.statusCode);
      } else if (response.statusCode >= 400 && response.statusCode <= 410) {
        print("Error: error");
        log("AuditByDevice Res22:::" "${response.body.toString()}");
        return GetItemCodeModel.issue(response.statusCode,
      ...
      return GetItemCodeModel.error(resCode, "$e");
    }
  }
}
